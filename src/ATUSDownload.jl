...
"""
    download_atus_data(data_file, year; save_dir="data")

Download the given ATUS data file from the doing-right-now S3 bucket. This saves the zip file in a subdirectory called "data". Returns nothing.
...
- `save_dir`: The directory to save the data in. Defaults to "data".

# Return
If `return_save_path` is true, returns the save_file_path string. Otherwise, returns nothing.
...
# Note
The ATUS zip files have been coppied as-is from the ATUS website and saved to an S3 bucket. To download from the bucket, the appropriate credentials must be in the system environment.
"""
function download_atus_data(data_file, year; save_dir="data", return_save_path=true)
    year = string(year)
    zip_file_name = "atus$(data_file)-$(year).zip"
    
    # Check if ./data exists. If not, make it.
    if !ispath(save_dir)
        mkdir(joinpath(save_dir))
    end

    # Check if the file is already there. If so, don't download it. If not, download and save it.
    if isfile(joinpath(save_dir, zip_file_name))
        return return_save_path ? joinpath(save_dir, zip_file_name) : nothing
    else
        response = S3.get_object("doing-right-now", zip_file_name)
        f = open(joinpath(save_dir, zip_file_name), "w") do file
                write(file, response)
        end
...
"""
    extract_dat_to_csv(zip_file_path, save_file_path)

Extract the ".dat" data file from the zipped directory and save it to the save_file_path location with a ".csv" extension. Returns nothing.

# Arguments
- `zip_file_path`: The path to the zip file.
- `save_file_path`: The path to save the extracted file to.
- `return_save_path`: Whether to return the save_file_path. Defaults to true.

# Returns
If `return_save_path` is true, returns the save_file_path string. Otherwise, returns nothing.
"""
function extract_dat_to_csv(zip_file_path, save_file_path; return_save_path=true)
    
    z = ZipFile.Reader(zip_file_path)
    file_names = [file.name for file in z.files]
    dat_file_index = findall(occursin.(r".+\.dat", file_names))[1]

    # Write the file to the save_file_path location.
    write(save_file_path, z.files[dat_file_index])
    close(z)
...
"""
    load_csv_to_dataframe(csv_file_path)

Load the csv file at the given path into a dataframe. Returns the dataframe.
"""
function load_csv_to_dataframe(csv_file_path)
    return CSV.File(csv_file_path) |> DataFrame
end
